    ...
    while (currentPage <= maxPages) {
      ...
      const pageProducts = await page.evaluate((amazonBaseUrl, usdToInrRate) => {
        return Array.from(document.querySelectorAll('.s-main-slot .s-result-item')).map(item => {
          ...
          const image = link ? `${amazonBaseUrl}${link}` : null;

          const priceWhole = item.querySelector('.a-price .a-price-whole')?.innerText.replace(/,/g, '').trim();
          const priceFraction = item.querySelector('.a-price .a-price-fraction')?.innerText.trim();
          const priceInUSD = priceWhole && priceFraction ? parseFloat(`${priceWhole}.${priceFraction}`) : null;
          const priceInINR = priceInUSD ? `₹${(priceInUSD * usdToInrRate).toFixed(2)}` : null;

          const rating = item.querySelector('.a-icon-alt')?.innerText.trim();

          if (title && priceInINR && image) {
            return { title, price: priceInINR, image, rating };
          }
          return null;
      ...
      products = products.concat(pageProducts);
      console.log(`Amazon Page ${currentPage} scraped: ${pageProducts.length} products.`);
